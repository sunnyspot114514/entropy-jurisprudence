...
**关键发现：**
- **Llama3 在 Ancient_Tree 上表现出极端合理化**（RI=328.59）
- **Qwen3 和 Mistral 最稳定** — 多个案例 RI=0
- **Gemma3 表现出"道德僵化"** — 无论案例如何，97.5% 判有罪
- **Phi3（最小模型）最不稳定** — 30% R 值幻觉率
...
| 模型 | 案例 | T=0.3 标准差 | T=0.6 标准差 | T=0.9 标准差 |
|------|------|-------------|-------------|-------------|
...
**关键洞见：** Qwen3 对温度免疫（所有温度下标准差 < 2），而 DeepSeek 在高 R 案例上表现出高方差。
